...
<ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="overlayOrigin" [cdkConnectedOverlayOpen]="isOpen">
  ...
  providers: [{
    ...
    useExisting: MuiSelect,
    multi: true
  }]
})
export class MuiSelect extends ControlValueAccessorConnector implements AfterViewInit, OnDestroy, OnInit {
  ...
  @ContentChildren(MuiOption)
  options: QueryList<MuiOption>;
  ...
  private keyManager: ActiveDescendantKeyManager<MuiOption>;
  ...
  chooseOption(event: MouseEvent, option: MuiOption) {
    // prevent bubbling
  ...
  private selectOption(option: MuiOption) {
